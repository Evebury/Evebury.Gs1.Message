...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template match="*[namespace-uri()='urn:gs1:gdsn:warranty_information:xsd:3' and local-name()='warrantyInformationModule']" mode="module">
		<xsl:param name="targetMarket"/>
		<xsl:param name="tradeItem"/>

		<xsl:apply-templates select="warrantyInformation/referencedFileInformation" mode="warrantyInformationModule">
			<xsl:with-param name="targetMarket" select="$targetMarket"/>
		</xsl:apply-templates>

	</xsl:template>

	<xsl:template match="referencedFileInformation" mode="warrantyInformationModule">
		<xsl:param name="targetMarket"/>
